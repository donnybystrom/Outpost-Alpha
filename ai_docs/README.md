...
The center of the clearing is reserved for the Planet Lander at sandbox start. Its flying model descends from above the map with six realtime particle jets and spatial procedural engine audio that intensifies toward the ground. The landed model is warmed behind the sandbox loading screen so touchdown does not synchronously load its mesh and textures. The reserved footprint blocks building placement and pathfinding during descent, but its initial metal and hauler delivery endpoint remain unavailable until touchdown. At touchdown the engine cuts into a generated mechanical thud, metallic resonance, and pneumatic pressure-release sound; the renderer swaps to the landed model, releases the transient flight visuals, and the module becomes the colony's operational command center and metal store, replacing the former starting HQ. Other modeled buildings begin threaded warmup on construction-button hover or focus, and tool activation waits for any remaining warmup instead of stalling the render frame.

Touchdown also deploys three selectable Space Marines around the Planet Lander. Their Meshy `Run_03` GLB supplies the skinned mesh, skeleton, and movement animation, while the runtime renderer reapplies the PBR textures from the authoritative Hyper3D base model. The rig and material workflow is documented in `ai_docs/gdd/animated-unit-models.md`.
